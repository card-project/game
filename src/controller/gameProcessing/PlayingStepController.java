...
public class PlayingStepController extends StepController {
	...
	/**
	 * Playing card method. Allow a player to play a card.
	 * 
	...
	public boolean currentPlayerCanPlay() {
		return this.currentGame.getCurrentPlayer().canPlay(
				super.currentGame.getOpponents( this.currentGame.getCurrentPlayer() ), super.currentGame.getGoal() );
	}
	...
	// public boolean performHumanPlayingStep(Card chosenCard) throws
	// AvailableCoupFourreException {
	//
	// boolean replay = false;
	//
	// if ( this.currentGame.getCurrentPlayer().canPlay(
	// super.currentGame.getOpponents( this.currentGame.getCurrentPlayer() ),
	// super.currentGame.getGoal() ) ) {
	//
	...
	// chosenTarget = this.chooseTarget( this.currentGame.getCurrentPlayer(),
	// (HazardCard) chosenCard );
	// userChoiceIsCorrect = ( ( HazardCard ) chosenCard ).isPlayableOn(
	// chosenTarget );
	// } else {
	// chosenTarget = this.currentGame.getCurrentPlayer();
	// if ( chosenCard instanceof DistanceCard ) {
	// userChoiceIsCorrect = ( ( DistanceCard ) chosenCard ).isPlayableOn(
	// chosenTarget, super.currentGame.getGoal() );
	// } else if ( chosenCard instanceof RemedyCard ) {
	// userChoiceIsCorrect = ( ( RemedyCard ) chosenCard).isPlayableOn(
	// chosenTarget );
	// } else if ( chosenCard instanceof SafetyCard ) {
	// userChoiceIsCorrect = ( ( SafetyCard ) chosenCard).isPlayableOn(
	// chosenTarget );
	// }
	// }
	//
	...
	//
	// replay = this.currentGame.getCurrentPlayer().play( chosenCard,
	// chosenTarget, super.currentGame.getGoal() );
	//
	// } else {
	// // TODO
