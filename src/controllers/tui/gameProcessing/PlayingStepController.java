...
public class PlayingStepController extends StepController {
			...
			do {
				...
				if ( chosenCard instanceof HazardCard ) {
					...
					if ( chosenCard instanceof DistanceCard ) {
						userChoiceIsCorrect = ( ( DistanceCard ) chosenCard ).isPlayableOn( chosenTarget,
	 ...
	 * 
	 * @param p
	 *            Player aiming to choose a card to play.
	 * @return Chosen card.
	 */
	private Card chooseCardToPlay( Player p ) {
		...
		do {
			...
			try {
				cardIndex = tui.askPlayingCard( p.getHand().toString() );
			...
			if ( userChoiceIsCorrect ) {
				if ( cardIndex < HandStack.MIN_CARD_NB || cardIndex > HandStack.MAX_IN_PLAY_CARD ) {
					...
					tui.warn( "Please enter a number between " + HandStack.MIN_CARD_NB + " and "
							+ HandStack.MAX_IN_PLAY_CARD );
					userChoiceIsCorrect = false;
				}
			}
		...
		return p.getHand().get( cardIndex - 1 );
	}

	/**
	 * @param currentPlayer
	 *            The current {@link Player} (in order to determine who are the
	 *            opponents).
