...
public class TUIGameController {
	...
	/**
	 * Unique public object method, it allows the user to play the game.
	 * 
	...
	/**
	 * Main game loop. Chain the drawing, playing and discarding steps.
	 * 
	 * @param firstPlayerIndex
	 *            Index of the first player.
	 */
	public void playCurrentGame( int firstPlayerIndex ) {
		...
		do {
			...
			try {
				...
				replay = e.getSafety().playCoupFourre( e.getCoupFourreInitiator(), currentPlayer,
						e.getHazardCardInitiator(), currentGame.getGoal() );
	...
	private void discardCard( Player p ) {
		if ( p.getHand().size() > HandStack.MAX_CARD_NB ) {
			tui.inform( '\n' + "-- > DISCARDING STEP" + '\n' );
