...
public class SafetyStackTest {
	...
	public void testPush() {
		SafetyCard correctCard = (SafetyCard) CardFactory.createCard( CardType.DrivingAce );
		DistanceCard distanceCard = (DistanceCard) CardFactory.createCard( CardType.Distance25 );
		HazardCard hazardCard = (HazardCard) CardFactory.createCard( CardType.Accident );
		RemedyCard remedyCard = (RemedyCard) CardFactory.createCard( CardType.Repairs );
	...
	public void testIsProtectedFrom() {
		...
		try {
			safetyStack.push( ( SafetyCard )CardFactory.createCard( CardType.DrivingAce ) );
			assertTrue( safetyStack.isProtectedFrom( CardFamily.StateOfCar ) );
			
			safetyStack.push( ( SafetyCard )CardFactory.createCard( CardType.RightOfWay ) );
			assertTrue( safetyStack.isProtectedFrom( CardFamily.GoStop ) );
			assertTrue( safetyStack.isProtectedFrom( CardFamily.Speed ) );

			safetyStack.push( ( SafetyCard )CardFactory.createCard( CardType.ExtraTank ) );
			assertTrue( safetyStack.isProtectedFrom( CardFamily.Gas) );

			safetyStack.push( ( SafetyCard )CardFactory.createCard( CardType.PunctureProof ) );
