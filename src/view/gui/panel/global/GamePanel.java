...
import view.gui.panel.Constant;
import view.gui.panel.assets.CarAsset;
import view.gui.panel.assets.CardIcon;
import view.gui.panel.assets.DiscardLayer;
import view.gui.panel.assets.DrawingStepLayer;
import view.gui.panel.assets.HandAndTargetLayer;
import view.gui.panel.assets.MapLayer;
...
public class GamePanel extends BasicPanel {
	...
	private void setPlayerHand() {
		...
		handContainer.setBounds(
			this.getSize().width - (cardDimension.width * 4) - 20,
			20,
			(cardDimension.width * 4),
			rows * (this.getSize().height / 6)
		);
		...
		if(handStack.size() > 0)
		{
			for (Card card : handStack) {
				...
				icon.setBounds(
					( 3 - i ) * cardDimension.width,
					j * cardDimension.height,
					cardDimension.width,
	...
	/**
	 * Configure the panel. Allows the assets to suit perfectly
	 * with the panel.
	 */
	...
	private void drawGameboard() {
		...
		Dimension ajustedDimension = new Dimension(
				(int)( this.corrector * Constant.GAMEBOARD.width ),
	...
	public void displayDeck() {
		...
		this.deckLayer = new DrawingStepLayer(this.getScreenDimension().width, this.getScreenDimension().height, this.controller);
	...
	public void displayHandLayer(Player player) {
		this.handLayer = new HandAndTargetLayer(this.getScreenDimension().width, this.getScreenDimension().height, this.controller);
	...
	public void displayDiscardChoice(Player player) {
		this.discardLayer = new DiscardLayer(this.getScreenDimension().width, this.getScreenDimension().height, this.controller);
	...
	private void setMiniMap(int number) throws TranscoderException, URISyntaxException {
		...
		Dimension mapSize = new Dimension(
			this.getScreenDimension().width,
			60
		);
		
		Point mapPosition = new Point(
			0,
			this.getScreenDimension().height - mapSize.height
		);
	...
	private void setCarsAssets(int number) throws TranscoderException, URISyntaxException {
		...
		int interval = (int) ((Constant.MAX_POSITION_CAR.y*this.corrector - Constant.MIN_POSITION_CAR.y*this.corrector) / number);
		
		Point position = null;
		
		Dimension dim = new Dimension(
				( int ) (this.corrector * Constant.CAR_DIMENSION.width),
		...
		for (int i = number; i >= 0 ; i--) {
			...
			position = new Point(
					0,
					( int ) (this.getScreenDimension().height - Constant.MAX_POSITION_CAR.y * this.corrector + i*interval)
			);
	...
	public void animatePlayer(Player p) {
		...
		int distance = ((this.background.getWidth() - icon.getSize().width) * p.getTraveledDistance()) / this.controller.getGameModel().getGoal();
