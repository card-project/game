...
public class Main {
	...
	private static void humanPlay( Game g, HumanPlayer player ) {
		...
		if ( g.getDiscardStack().isEmpty() ) {
			System.out.println( "Discard stack is empty, deck stack is automatically chosen." );
			Card c = player.draw( g.getDeckStack() );
			System.out.println( "DRAWN : " + c );
		} else if ( g.getDeckStack().isEmpty() ) {
			System.out.println( "Deck stack is empty, discard stack is automatically chosen." );
			Card c = player.draw( g.getDiscardStack() );
			System.out.println( "DRAWN : " + c );
		} else {
		...
		do {
			...
			try {
				...
				// FIXME ugly
				chosenCard = player.pickCardToPlay( Integer.valueOf( new Scanner ( System.in ).nextLine() ) );
		...
		// STEP 2.2 : If the card is an hazard, choose its target
		...
		// STEP 2.3 : Check whether the move is allowed
		try {
