...
public class DrawingStepLayer extends BlackLayer implements CardAndTargetListener {
	...
	private void fillDeck( Game game ) {
		...
		Point position = new Point( this.container.getLocation().x + 340, this.container.getLocation().y + 60 );

		nullCard.setBounds( position.x, position.y, Constant.DECK_CARD_DIMENSION.width,
				Constant.DECK_CARD_DIMENSION.height );
		...
		if ( !game.getDiscardStack().isEmpty() ) {
			...
			position = new Point( this.container.getLocation().x + 60, this.container.getLocation().y + 60 );

			discardCard.setBounds( position.x, position.y, Constant.DECK_CARD_DIMENSION.width,
					Constant.DECK_CARD_DIMENSION.height );
	...
	private void defineDeck() {
		...
		try {
			...
			container.setBounds( this.dimension.width / 2 - Constant.DECK_DIMENSION.width / 2, this.dimension.height
					/ 2 - Constant.DECK_DIMENSION.height / 2, ( int ) ( Constant.DECK_DIMENSION.width ),
