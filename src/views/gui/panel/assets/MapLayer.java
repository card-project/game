...
public class MapLayer extends JLayeredPane {
	...
	public MapLayer ( Dimension size, Point position, int numberOfPlayers, Dimension carDimension )
			throws TranscoderException, URISyntaxException {
		this.size = size;
	...
	private void setCarsAssets( int number ) throws TranscoderException, URISyntaxException {
		...
		Dimension dim = new Dimension( ( int ) ( corrector * carDimension.width ),
				( int ) ( corrector * carDimension.height ) );

		for ( int i = number - 1 ; i >= 0 ; i-- ) {

			CarAsset c = new CarAsset( i, dim );
			c.setBounds( dim.width * i, 10, dim.width, dim.height );
