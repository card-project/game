...
public class CardIcon extends JLabel implements MouseListener {
	...
	private void createIcon() {
		...
		try {
			...
			if ( this.card != null )
				icon = new SVGIcon( new File( new String( "assets/" + this.card.getType() + ".svg" ) ).toURI()
						.toString(), rescaledDimension.width, rescaledDimension.height );
			else
				icon = new SVGIcon( new File( new String( "assets/Null.svg" ) ).toURI().toString(),
