...
public class HandAndTargetLayer extends BlackLayer implements CardAndTargetListener {
	...
	private void setOpponentContainer() {
		...
		this.opponentContainer.setBounds( OFFSET, OFFSET, dimension.width - 2 * OFFSET, ( dimension.height / 2 ) - 2
				* OFFSET );
	...
	private void drawOpponents( HazardCard card ) {
		...
		ArrayList<Player> opponentList = this.controller.getGameModel().getAttackableOpponents(
				this.controller.getGameModel().getCurrentPlayer(), card );

		Dimension dim = new Dimension( ( dimension.width - 2 * OFFSET ) / opponentList.size(), ( dimension.height / 2 )
				- 2 * OFFSET );
		...
		for ( Player p : opponentList ) {
			...
			icon.setBounds( OFFSET + i * ( dim.width ), OFFSET,
	...
	private void setSwitchToDiscard() {
		...
		try {
			SVGIcon icon = new SVGIcon( new File( "assets/text/wantDiscard.svg" ).toURI().toURL().toString(),
					Constant.WANTDISCARD_TEXT.width, Constant.WANTDISCARD_TEXT.height );
			this.discardLabel = new JLabel( icon );
			this.discardLabel.setBounds( OFFSET, OFFSET, Constant.WANTDISCARD_TEXT.width,
	...
	private void defineHand( Player player ) {
		...
		Dimension cardDimension = new Dimension( ( dimension.width - 2 * OFFSET ) / numberOfCards,
				( dimension.height / 2 ) - 2 * OFFSET );

		for ( Card card : playerHand ) {
			CardIcon c = new CardIcon( card, cardDimension, this );

			c.setBounds( OFFSET + i * ( cardDimension.width ), OFFSET + this.getSize().height / 2,
	...
	public void cardChosen( CardChosenEvent event ) {
		...
		else if ( !this.controller.playCard( this.chosenCard, this.controller.getGameModel().getCurrentPlayer() ) )
