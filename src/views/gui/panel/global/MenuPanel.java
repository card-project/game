...
public class MenuPanel extends BasicPanel {
	...
	public void setSkyBackground() {
		...
		int paddingTopBackground = - ( this.getScreenDimension().height - ( this.getScreenDimension().width * O_HEIGHT )
				/ O_WIDTH );
	...
	public void setLandscape() throws TranscoderException {
		...
		int paddingTopLandscape = ( this.getScreenDimension().height > height ) ? Math.abs( height
				- this.getScreenDimension().height ) : - ( height - ( this.getScreenDimension().height ) );
	...
	public void setCounterBar() throws TranscoderException {
		...
		try {
			onePlayerButton = this.createButton( getClass().getResource( "/onePlayerCounter.svg" ).toURI(),
					( wPanel / 4 ) );
			multiPlayerButton = this.createButton( getClass().getResource( "/multiplayerCounter.svg" ).toURI(),
					( wPanel / 4 ) );
	...
	public void setLogo() throws TranscoderException {
		SVGIcon logo = new SVGIcon( this.logoFile.toURI().toString(), this.getScreenDimension().width / 3,
				this.getScreenDimension().height / 3 );
		this.logo = new JLabel( logo );
		this.logo.setBounds( this.getScreenDimension().width / 2 - logo.getIconWidth() / 2, 0, logo.getIconWidth(),
				logo.getIconWidth() );
	}
